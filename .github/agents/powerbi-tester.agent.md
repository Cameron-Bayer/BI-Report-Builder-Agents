...
### Additional Error Patterns from TriageServiceBugReport

#### Error: "Property 'isNameInferred' is unknown"
**Root cause**: `isNameInferred` or `sourceColumn: [Value]` used on a custom column. These are only valid on auto-generated date hierarchy columns (DateTableTemplate, LocalDateTable).
**Diagnostic**: Search all .tmdl files for `isNameInferred` — if found on any table other than DateTableTemplate/LocalDateTable, flag it.
**Fix guidance**: Remove the property. Use `sourceColumn: ColumnName` instead.

#### Error: "A cyclic reference was encountered during evaluation"
**Root cause**: DAX calculated column (`column X = [Value]`) on a calculated table (`partition T = calculated`) that participates in relationships. The DAX evaluation cycle: column depends on table expression → table is filtered by relationship → relationship references column.
**Diagnostic**: Find any `partition ... = calculated` tables that have relationships in `relationships.tmdl` AND have `column X = [Value]` syntax.
**Fix guidance**: Convert the calculated table to an M partition (`partition T = m`). This is the only reliable fix.

#### Error: "Relationship '<guid>' uses an invalid column ID"
**Root cause**: `sourceColumn` doesn't resolve correctly on DAX calculated tables. The internal column ID mapping differs from import tables.
**Diagnostic**: Find relationships where `toColumn` or `fromColumn` references a column on a calculated table.
**Fix guidance**: Convert the calculated table to an M partition.

#### Error: "Table with ShowAsVariationsOnly=1 must be a target of a variation"
**Root cause**: A LocalDateTable .tmdl file exists but the `variation` block that referenced it was removed from the parent column. Must clean up all 4 artifacts: variation block, relationship, ref table entry, and LocalDateTable .tmdl file.
**Diagnostic**: Find any LocalDateTable .tmdl files and verify each one is referenced by a `variation` block in a parent table column.
